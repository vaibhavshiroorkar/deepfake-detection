...
from models.streams.common.config import (
    StreamConfig, efficientnet_config, xception_config, dinov3_config,
    EFFICIENTNET_B0, XCEPTION, DINOV3,
)
...
def test_dinov3_preset_pools_the_class_token():
    """Offline guard for the bug that made dinov3_config() unusable.

    global_pool="avg" makes timm build `fc_norm` where DINOv3's released weights
...
@pytest.mark.parametrize("preset", [efficientnet_config, xception_config, dinov3_config])
def test_pretrained_weights_actually_load(preset):
    ...
    which is exactly why the DINOv3 pooling bug shipped undetected: the
...
def test_dinov3_feature_dim_matches_the_documented_384():
    ...
    """
    try:
        model = VisualStream(dinov3_config(grad_checkpointing=False, frame_chunk_size=0))
    except Exception as e:                      # noqa: BLE001 - re-raised unless offline
        _skip_if_weights_unreachable(e)
        return

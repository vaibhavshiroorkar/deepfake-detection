...
- DINOv2, a higher-level view that generalizes better to unseen fakes (fallback: a Swin Transformer, if DINOv2's self-supervised math/integration is too heavy for the deadline — but note Swin is *supervised* and won't match DINOv2's generalization to unseen fakes, so it's a lower-effort substitute, not an equal one; see [docs/math/swin.md](docs/math/swin.md))

...
A segmented control switches which section renders:
  - Config: pick the clip (Selection) and global settings, and see the model-input
    contract + a copyable summary of the step toggles set in the other two views.
    The chosen clip and settings are cached in session_state for the other views.
...
if view == "Config":
    ...
    # Cache the selection so the Visual and Audio views can read it.

...
def backbone_stages(backbone) -> list[StageSpec]:
    ...
    return [
        StageSpec(name=name, channels=0, reduction=0)
...
def token_grid(num_tokens: int) -> tuple[int, int]:
    ...
    for prefix in range(MAX_PREFIX_TOKENS + 1):
        ...
        if side * side == patches:
            return prefix, side
    raise ValueError(
        f"{num_tokens} tokens do not decompose into a prefix plus a square grid"
    )
...
def trace_visual_stream(
    model, frames: torch.Tensor, detail_frame: int = 0
) -> StreamTrace:
    ...
    if frames.ndim != 5 or frames.shape[0] != 1:
        raise ValueError(
            f"trace needs one clip shaped [1, T, 3, H, W], got {tuple(frames.shape)}"
        )
    num_frames = frames.shape[1]
    if not 0 <= detail_frame < num_frames:
        raise ValueError(
            f"detail_frame {detail_frame} outside the clip's {num_frames} frames"
        )
    ...
    handles = [
        model.backbone.get_submodule(r.spec.name).register_forward_hook(r)
        for r in recorders
    ]
...
def _final_hidden(model, hidden) -> torch.Tensor:
    ...
    h_last = h_n.view(
        model.config.temporal_layers, num_dir, 1, model.config.temporal_hidden
    )[-1]

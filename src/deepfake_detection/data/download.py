...
def parse_args():
    ...
    if server == "EU":
        ...
        raise Exception(f"Wrong server name. Choices: {str(SERVERS)}")
...
def reporthook(count, block_size, total_size):
    ...
    sys.stdout.write(
        f"\rProgress: {percent:d}%, {progress_size / (1024 * 1024):.0f} MB, "
        f"{speed:d} KB/s, {duration:.0f} seconds passed"
    )
    sys.stdout.flush()
...
def download_file(url, out_file, report_progress=False):
    ...
    if not os.path.isfile(out_file):
        ...
        tqdm.write("WARNING: skipping download of existing file " + str(out_file))
        ...
        if c_type == "videos":
            ...
            if "original" in dataset:
                if args.dataset != "all":
                    ...
                    print(
                        "Only videos available for original data. Skipping original.\n"
                    )
                    continue

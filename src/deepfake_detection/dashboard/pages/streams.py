...
The settings live in session_state via the dashboard's lib/stream_pages.py, so a
subpage opens with whatever was set here.
"""

import streamlit as st

from deepfake_detection.dashboard.lib import stream_pages
from deepfake_detection.dashboard.lib.stream_spec import STREAMS
...
c1, c2 = st.columns(2)
for column, (key, (name, encoders, stage)) in zip(
    (c1, c2), stream_pages.CROSS_MODAL.items(), strict=True
):
...
if enabled:
    st.code(" + ".join(f"{stream_pages.VISUAL_MODELS[k][0]} [{d}]"
                       for k, d in zip(enabled, dims, strict=True))

...
Sources of truth it mirrors: docs/research-design.md, docs/data-card.md and
docs/model-selection.md. The last tab links every repository record rather than
restating it, so a reader lands on the file that is actually maintained.
    ...
    with diagram_col:
        if DIAGRAM.exists():
            ...
            st.warning(f"Architecture diagram not found at "
                       f"`{DIAGRAM.relative_to(PROJECT_ROOT)}`.")
...
def render_pipeline():
    ...
    st.markdown("""
Two entry points share one implementation. `ddf cache build` is the batch
path, called per clip by `dataset.ClipDataset` and warmed in bulk by `preprocessing/precache.py`.
The Preprocessing page calls the same `deepfake_detection/preprocessing/ops/` functions one step at a time so each
...
| # | Stage | Code | Result |
|---|---|---|---|
| 0 | Locate the dataset | `ddf manifest build` | the directory holding `meta_data.csv` |
| 1 | Build the manifest | `manifest.manifest_from_meta`, `manifest.clip_label` | one row per clip |
| 2 | Split on identity | `ddf split build` | `train/val/test.csv` |
...
| 10 | Write the cache | `ddf cache build` | one `.npz` per clip under the run's `cache/` |
...
`AUDIO_WINDOW_SEC=0.35`, `PIPELINE_VERSION=4`.
""")
...
player re-encodes (`the dashboard's lib/media.playable_video_bytes`).""")
...
`ddf split build` reports zero identity overlap and zero file overlap, and is the gate that makes
...
timestamps = sample_timestamps(duration, 16, 0.35, start_offset=silence)""", language="python")
        st.info("The waveform is never cut. Trimming shortens the audio while the frame timestamps "
                ...
                "``ddf manifest build`` also records the measurement per clip as `leading_silence_sec`, "
...
reports it as *Worst off-centre*. `ddf cache build` behaves identically, so correcting it would
...
removed, and the two traps it carried are recorded in `docs/obstacles.md`.
...
it, but `ddf cache build` binds it to `_mouth` and never writes it, so the cache holds faces and audio
...
""")
    ...
    with st.expander("**Stage 10 · The cache**"):
        st.markdown("""
One compressed `.npz` per clip, written by `CacheStore` under the run's cache root as
`<dataset>-<hash>/<clip id>/<preprocessing fingerprint>.npz`. The fingerprint in the filename is
what makes a stale cache detectable: a clip preprocessed under different settings lands at a
different path instead of overwriting the old one.
""")
        st.code("""visual_view        (16, 3, 224, 224)  float32   # CHW, ImageNet-normalised
audio_view         (64000,)           float32   # 4 s @ 16 kHz
sync_video_view    (50, 3, 112, 112)  float32   # mouth crops for the sync branch
sync_audio_view    (32000,)           float32   # 2 s @ 16 kHz
sync_audio_context (42240,)           float32   # padded window around it
metadata           JSON                         # quality report and fingerprints""", language="text")
        st.markdown("""
...
label  (B,)                  int64     # per label_mode""", language="text")
        st.info("**The audio is never normalised in the contract.** Nothing in `ops/audio.py` or "
                "`ddf cache build` touches amplitude, so `audio.npy` keeps whatever level the source "
...
| Cache | Written to the run's cache root | Never written; the page is read-only |
...
| Shipped by | `facenet-pytorch` | OpenCV, weights in `models/`, fetched by `ddf detector fetch-yunet` |
...
""")
        st.info("**Five-point alignment used to sit here and was removed.** It warped the face onto "
                ...
                "frame. The two traps it carried, the black-padding fix and the template inset, are "
                "recorded in `docs/obstacles.md`, so bringing it back does not start from "
...
`leading_silence_sec` by ``ddf manifest build``, and that offset is passed into `sample_timestamps`
...
`ddf split build`, whose audit reports zero identity and zero file overlap, and never re-split randomly
...
| `runs/<run>/cache/` | Per-clip `.npz` cache, keyed by dataset, clip id and preprocessing fingerprint. Gitignored. |
| `data/*.csv` | `full_manifest.csv` and the `train` / `val` / `test` splits. |
| `models/face_detection_yunet_2026may.onnx` | YuNet's ONNX weights, fetched by `ddf detector fetch-yunet` and integrity-checked. MTCNN is the default and needs no fetch. |
...
side by side. `ddf manifest build` and the dashboard's picker apply the
...
(`the dashboard's lib/media.playable_video_bytes`), captions the clip when it has done so, and leaves the
...
``ddf manifest build`` has ever run. And a manifest CSV is attached to whichever dataset its
...
def render_records():
    st.header("The records this page cannot replace")
    st.markdown(
        "Everything above explains how a mechanism works. These files record what "
        "was actually run, on which data, and what the result is allowed to claim. "
        "They are the maintained source; this page is a reading of them."
    )
    for label, target in DOCUMENTS:
        st.markdown(f"- [{label}]({target})")
    st.caption(
        "Obstacles hit while building each part are collected in docs/obstacles.md, "
        "linked from the project handoff."
    )
...
tabs = st.tabs(["Problem & architecture", "Input → tensors", "Visual path", "Audio path",
                ...
                "Repository records"])
renderers = [render_architecture, render_pipeline, render_visual, render_audio,
             render_streams, render_fusion, render_data, render_records]

...
either way, so callers do not care which was used:
  ...
  - **Dataset**: pick a DATASET, then a SPLIT within it, then a clip through the
    ...
    the dashboard's lib/datasets.py for the detection rules).
...
def filter_manifest(
    df: pd.DataFrame, manip_types: list[str], methods: list[str], label_filter: str
) -> pd.DataFrame:
    out = df
...
def group_by_identity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = [group_col] + (
        ["manipulation_type"] if "manipulation_type" in df.columns else []
    )
...
def _render_dataset(st):
    ...
    if not found:
        st.info(
            f"No datasets found in `{DATA_DIR}`. Drop a dataset folder there "
...
def _selected_card(st, row, action_label: str | None):
    ...
    with st.container(border=True):
        ...
        if row is None:
            ...
            mtype = (
                row["manipulation_type"] if "manipulation_type" in row else "unknown"
            )
...
def _picker(st, df):
    ...
    def _body():
        ...
        with left:
            ...
            manip = (
                f1.multiselect("Type", MANIP_TYPES, key="pick_types")
                if has_type
                else []
            )
            methods = (
            ...
            label_filter = f3.radio(
                "Label", ["all", "real", "fake"], horizontal=True, key="pick_label"
            )
            filtered = group_by_identity(
                search_manifest(
                    filter_manifest(df, manip, methods, label_filter), query
                )
            )
            st.caption(
                f"{len(filtered)} clips, grouped by identity. Click a row to load it."
            )
            view_cols = [
                c
                for c in ["clip_id", "source", "manipulation_type", "method", "label"]
        ...
        active = (
            df[df["clip_id"] == active_id] if active_id is not None else df.iloc[0:0]
        )
        ...
        with right:
            if candidate is None:
                ...
                cmt = (
                    candidate["manipulation_type"]
                    if "manipulation_type" in candidate
                    else ""
                )
...
def _preview_frames(st, video_path, n: int = 4):
    ...
    frames = [
        cv2.resize(f, (150, 150)) for f in media.cached_decode_frames(video_path, ts)
    ]
    cols = st.columns(2)

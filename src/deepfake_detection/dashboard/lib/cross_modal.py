...
from deepfake_detection.preprocessing.ops import audio as AF
from deepfake_detection.preprocessing.ops.constants import AUDIO_SR
...
def decode_inputs(
    ...
    faces, mouths, detected, _ms = media.cached_face_mouth(
    ...
    return {
        ...
        "duration": duration,
        "fps": fps,
        "has_audio": bool(raw.size),
    }


def render(st, spec: dict, key: str, video_source: str):
    """The whole page body. `video_source` is "mouths" or "faces"."""
    from deepfake_detection.dashboard.lib import stream_pages, stream_ui, trace_ui
    ...
    st.caption(
        "Produced by `preprocessing.ops`, the same code the batch pipeline runs. These "
    ...
    visual_label = (
        "Mouth crops, 96 pixels, derived from the two mouth-corner landmarks"
        if video_source == "mouths"
        else "Face crops, 224 pixels, margin-padded bounding box"
    )
    ...
    with st.container(border=True):
        ...
        left.caption(
            f"One {window_sec:.2f}s window per frame timestamp, at {AUDIO_SR} Hz."
        )
        if not data["has_audio"]:
            right.warning(
                "This clip carries no audio stream, so the windows below are silence."
            )
        right.pyplot(
            trace_ui.matrix_fig(
                np.abs(windows).T,
                f"Audio windows {windows.shape} (magnitude)",
    ...
    st.latex(
        r"\text{Attention}(Q,K,V) = \text{softmax}\!\left(\frac{QK^{\top}}"
        r"{\sqrt{d_k}}\right)V"
    )
    st.warning(
    ...
    evidence = random_projection(
        np.stack([v.astype(np.float32) / 255.0 for v in visual]),
        DEMO_DIM,
        DEMO_SEED + 1,
    )
    ...
    with left:
        ...
        st.metric(
            "Mass near the diagonal",
            f"{diagonal_mass(weights):.2f}",
        ...
        st.caption(
            "The stream hands fusion the attended vector, not a decision. Nothing is "
            "transcribed and no emotion label is produced; everything stays vectors."
        )
    with right:
        st.pyplot(
            trace_ui.attention_fig(
                weights,
                "Attention weights (random projections)",
                f"{spec['keyvalue']} step",
                f"{spec['query']} step",
            )
        )

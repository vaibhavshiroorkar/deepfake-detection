...
deepfake_detection/streams/introspect.py captures what a model computed and returns
...
# The page is dark, and matplotlib defaults to a white canvas with black text,
# which lands as a bright rectangle in the middle of every panel. Transparent
# rather than a fixed dark colour, so a figure sits on whatever the page is
# painting behind it. Applied at import: every figure in the dashboard is built
# through this module or a page that imports it.
_INK = "#E6EDF3"
_MUTED = "#8FA3B3"
plt.rcParams.update(
    {
...
def heatmap(
    map2d: np.ndarray, size: int = DISPLAY_SIZE, cmap: str = ACTIVATION_CMAP
) -> np.ndarray:
...
def matrix_fig(
    matrix: np.ndarray, title: str, xlabel: str, ylabel: str, figsize=(10, 2.4)
):
...
def attention_fig(
    weights: np.ndarray, title: str, xlabel: str, ylabel: str, figsize=(5.2, 4.4)
):
